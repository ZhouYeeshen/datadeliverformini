      ...
      >
        <cover-view class="camera-overlay">
          ...
          <cover-view class="live-text" v-if="liveText">
            <cover-view class="live-text-label">已识别到文字：</cover-view>
...
function onCameraReady() {
  // Start VKSession for real-time text detection
  // @ts-ignore
  ...
  const { supported, stop } = initDeviceOCR(comp, (text: string) => {
    liveText.value = text.substring(0, 300)
  })
  if (!supported) {
    console.log('[OCR] VKSession not available, will use backend fallback')
  }
  vkStop = stop
}
...
async function doOCR() {
  ...
  try {
    ...
    // If VKSession found text, use device text + backend parsing
    if (liveText.value) {
      ocrProgress.value = 40
      result = await parseText(liveText.value)
      result.engine = 'device-ocr'
...
.live-text-label { color: #2979FF; font-size: 22rpx; display: block; }
